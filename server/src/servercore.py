...
class ServerCore:
    ...
    def __init__(self, items_db, users_db,
        ...
        items_db and users_db stands for data bases with users and items respectively.
        min_limit and max_limit are bonds for user initialization reward.
        save_frequency is how many operations must take place between automatic commits.
        Operations from each user counts.
        simultanious_log_ins is binary flag,
        which allows or forbide multiple users log in into one account simultaniously.
        """
        ...
        self._user_requests = {
            ...
            Request.Type.GET_USER_ITEMS: lambda user: {item.name: item for item in user.items}
        }

        self._complex_requests = {
            Request.Type.USER_EXISTS: lambda _, name:
                Responce(Request.Type.USER_EXISTS, data=(name in self._users)),
    ...
    def handle_request_from(self, user, request):
        ...
        if request_type in self._plain_requests:
            ret = Responce(request_type, data=self._plain_requests[request_type]())
        elif request_type in self._user_requests and not user:
            ret = self._no_user_responce(request_type)
        elif request_type in self._user_requests:
            ret = Responce(request_type, data=self._user_requests[request_type](user))
        else:
    ...
    def _user_has(self, user, item_name):
        if not user:
            return self._no_user_responce(Request.Type.USER_HAS)
        ...
        return Responce(Request.Type.USER_HAS, data=ret)

    def _get_item(self, user, item_name):
        if item_name not in self._items:
            return self._no_item_responce(Request.Type.GET_ITEM, item_name)
        
        return Responce(Request.Type.GET_ITEM, data=self._items[item_name])

    def _get_user(self, user, user_name):
        if user_name not in self._users:
            return Responce(Request.Type.GET_USER,
                            success=False,
                            message="No such user")
        return Responce(Request.Type.GET_USER, data=self._users[user_name])
    ...
    def _buy_item(self, user, item_name):
        ...
        self._operation_count += 1
        return Responce(request_type, message="Item bought: " + item_name)
    ...
    def _sell_item(self, user, item_name):
        ...
        self._operation_count += 1
        return Responce(request_type, message="Item sold: " + item_name)
